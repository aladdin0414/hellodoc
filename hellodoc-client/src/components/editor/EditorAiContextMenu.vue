...
const { t } = useI18n()
const { activeModels, currentModelId, fetchActiveModels, selectModel } = useAiModels()
...
<template>
    <BaseDialog :show="visible" @close="handleClose" max-width-class="max-w-[540px]"
        ...
        <div class="p-6">
            <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                ...
                <!-- 大模型选择器（仅显示已激活模型） -->
                <div v-if="activeModels.length > 0" class="flex items-center gap-1.5">
                    <span class="text-xs text-slate-400">模型:</span>
                    <select
                        :value="currentModelId"
                        @change="selectModel(($event.target as HTMLSelectElement).value)"
                        class="text-xs bg-slate-100 dark:bg-gray-800 text-slate-700 dark:text-gray-300 rounded-lg px-2 py-1 border border-slate-200 dark:border-gray-700 focus:outline-none focus:ring-1 focus:ring-indigo-500 max-w-[170px] truncate"
                    >
                        <option
