      ...
      <template #right>
        ...
        <div v-else class="text-xs flex items-center gap-1 select-none transition-all">
          <template v-if="saving || uploadingImage">
            ...
            <span class="text-slate-400">{{ uploadingImage ? '图片上传中' : '保存中' }}</span>
          </template>
    ...
    <main class="flex-1 flex flex-col px-5 py-4 w-full space-y-3 pb-[calc(5.5rem+env(safe-area-inset-bottom))]">
      ...
      <template v-else>
        ...
        <div class="flex items-center gap-1.5 pt-1 pb-1">
          <span v-if="isDirty && isEditing && !isReadOnly" class="text-rose-500 text-3xl font-extrabold shrink-0 select-none" title="有未保存修改">*</span>
          <input
            v-model="docTitle"
            @input="markDirty"
            type="text"
      ...
      <div v-if="isEditing" class="flex-1 flex flex-col space-y-2">
        ...
        <div class="flex items-center gap-1.5 overflow-x-auto py-1.5 text-xs no-scrollbar">
          ...
          <button @click="applyInlineSyntax('**', '**', '粗体')" class="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold active:scale-95 transition-all shrink-0">B</button>
          <button @click="applyBlockSyntax('- ')" class="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 active:scale-95 transition-all shrink-0">• 列表</button>
          <button @click="applyBlockSyntax('> ')" class="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 active:scale-95 transition-all shrink-0">引用</button>
          <button @click="applyInlineSyntax('`', '`', '代码')" class="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 active:scale-95 transition-all shrink-0">&lt;&gt; 代码</button>
          <button
      ...
      <div v-else class="flex-1 min-h-[500px] py-2 pb-12">
        <VisualEditor
          ...
          :pure-mode="true"
        />
        <p v-else class="text-slate-400 italic py-4">（暂无正文内容）</p>
      </div>
      </template>
    </main>
  </div>
